// MxodKw7SN7PrR1m_i-p-pK8IDDx8yuSIQ
// https://developers.google.com/apps-script/guides/sheets/functions#optimization
// Кастомные функции для сбора конфигов необходимые для корректно работы гуглотаблички с примерами
...
function treeToConfig(arrays) {
  ...
  for (var i = 0; i < arrays.length; i++) {
    ...
    //Если выше одноранговый, разделяем запятой
    else if (lineLevel == upperLineLevel) {
      out += ', '
    }
    ...
    //Если ниже есть более высокоуровневый элемент, то надо отделить запятой
    if (lineLevel > lowerLineLevel) {
      out += ', '
    }
 ...
 * @param {text} text Скобки обрамляющие получившуюся строку. Обязательно указывать парой, например "{}"
 * @customfunction
 */
function joinStrings(sep, data, br) {
  var br = br || ""
  var out = data.map(function(row) {
    return row.filter(function(item){return isNotEmpty(item)}).join(sep)
  })

  if (br.length > 1 & any(out)) {
    var out = out.map(function(element) {
      if (any(element)) {
        return out = br[0] + element + br[1]
      }
    })
 ...
 *
 * @param {string} sepInt Разделитель через который склеиваются данные внутри строки
...
// Блок – каждая заполненную строку и только заполненная строка.
 ...
 * @customfunction
 */
function toString(names_array, data_array, br) {
  var br = br || ""
  var txt = br[0] || ""
...
/**
 * Расширение функции toString. Склеивает блоки с информацией в строки конфига. Разделение на блоки задается отдельно.
 ...
 * @customfunction
 */
function toStringCustomBlock(headers, block_info, data, block_function, br) {
  var out = []
  var intervals = []
  ...
  for (var i = 0; i < intervals.length; i += 2) {
    out[intervals[i]] = toString(headers, data.slice(intervals[i], intervals[i+1]), br)
  }
