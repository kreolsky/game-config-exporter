...
## Введение
Меня зовут Серёжа, я работаю гейм-дизайнером и люблю изобретать велосипеды, ради этого даже питон пришлось учить.
...
Средний блок с сюрпризом, да, но это расплата за простоту формата. Если внутри блока есть части словаря, то они будут собраны в один словарь. В моих реальных конфигах я на это ограничение ни разу не попадал.
...
Сам конвертов в JSON, с примерами, можно найти тут: bot/gss_exporter_bot/tools.py
...
* В остальных может быть что угодно, комментарии, расчеты. Столбцы с данными даже могут не находится рядом. В строках ниже идут данные, в key название переменных, в value -- значения. На выходе будет валидный [json документ](https://github.com/kreolsky/game-config-exporter/tree/master/json/general1.json)
...
Можно и не копировать, а создать свой, но советую таки скопировать, там уже подготовлена база, в том числе и из кастомных функций, которые знатно облегчают жизнь при работе со сложными конфигами. Каждая функция документирована. Не ленитесь залезть в редактор сриптов (Tools \ Script Editor) и посмотреть что там есть. Из одного документа в другой функции можно копировать либо подключать как библиотеку.
Внимательно посмотрите на файл с х[настройками для конфигов](https://docs.google.com/spreadsheets/d/1qAaCU0ERtuzPY6LNHU2FQSkkaBS_-gLmvjwzvW-lcaA), начать со вкладки read.me! и пройтись по остальным вкладкам, читаю комментарии, я старался подробно всё описать.
...
Запускайте скрипт, он должен достучаться до гуглтаблиц и сохранить все вкладки которые начинаются с "@".
